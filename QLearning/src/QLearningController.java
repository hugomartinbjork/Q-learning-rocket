...
public class QLearningController extends Controller {
	...
	Hashtable<String, Double> Qtable = new Hashtable<String, Double>(); /*
																		 * Contains the Q-values - the state-action
																		 * utilities
																		 */
	Hashtable<String, Integer> Ntable = new Hashtable<String, Integer>(); /*
																			 * Keeps track of how many times each
																			 * state-action combination has been used
																			 */
	...
	static final double LEARNING_RATE_CONSTANT = 10; /*
														 * See alpha(), lower values are good for quick results in large
														 * and deterministic state spaces
														 */
	double explore_chance = 0.5; /* The exploration chance during the exploration phase */
	final static int REPEAT_ACTION_MAX = 30; /*
												 * Repeat selected action at most this many times trying reach a new
												 * state, without a max it could loop forever if the action cannot lead
												 * to a new state
												 */
...
//		System.out.println("This is action :" + action + "    ||      Current angle: " + angle.getValue());
		...
		/* Fire zeh rockets! */
		/*
		 * TODO: Remember to change NUM_ACTIONS constant to reflect the number of
		 * actions (including 0, no action)
		 */
...
//			String new_state = StateAndReward.getStateAngle(angle.getValue(), vx.getValue(), vy.getValue());
			...
			/*
			 * Repeat the chosen action for a while, hoping to reach a new state. This is a
			 * trick to speed up learning on this problem.
			 */
			action_counter++;
...
//			double previous_reward = StateAndReward.getRewardAngle(previous_angle, previous_vx, previous_vy);
			...
			if (previous_state != null) {
				...
				double Qold=Qtable.get(prev_stateaction); 
				double alpa = alpha(Ntable.get(prev_stateaction));
//				System.out.println(previous_reward);
//				System.out.println(getMaxActionQValue(new_state) - Qtable.get(prev_stateaction) + " TD");
				double td=(previous_reward + (GAMMA_DISCOUNT_FACTOR * getMaxActionQValue(new_state)) - Qtable.get(prev_stateaction));
				double Qnew= Qold + alpa*td;
				Qtable.put(prev_stateaction, Qnew);
	...
	/*
	 * Computes the learning rate parameter alpha based on the number of times the
	 * state-action combination has been tested
	 */
	public double alpha(int num_tested) {
		/*
		 * Lower learning rate constants means that alpha will become small faster and
		 * therefore make the agent behavior converge to to a solution faster, but if
		 * the state space is not properly explored at that point the resulting behavior
		 * may be poor. If your state-space is really huge you may need to increase it.
		 */
	...
	/*
	 * The 'E' key will toggle the agents exploration mode. Turn this off to test
	 * its behavior
	 */
